	     ...
	     size_t idx_size, struct packed_git *p)
{
	...
	if (version == 1) {
		...
		p->crc_offset = 8 + 4 * 256 + (size_t)nr * hashsz;
		p->num_64bit_offsets =
			(idx_size - p->crc_offset - (size_t)nr * 8 - 2 * hashsz) / 8;
...
void check_pack_index_offset(const struct packed_git *p, uint32_t off)
{
	if (off >= p->num_64bit_offsets)
...
off_t nth_packed_object_offset(const struct packed_git *p, uint32_t n)
{
	...
	if (p->index_version == 1) {
		...
		uint32_t off;
		size_t tablesz = (size_t)p->num_objects * 4;
		index += p->crc_offset + tablesz;
		off = ntohl(*((uint32_t *)(index + 4 * (size_t)n)));
		if (!(off & 0x80000000))
			return off;
		off &= 0x7fffffff;
		check_pack_index_offset(p, off);
		index += tablesz + (size_t)off * 8;
