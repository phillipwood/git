...
+
Once the worktree has been created and _<commit-ish>_ has been checked
out the `worktree-add` hook is invoked with the path to the new
worktree.  This hook can be used to initalize untracked configuration
files or setup the project's build system in the new worktree.
