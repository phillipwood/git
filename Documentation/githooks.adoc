...
This hook is invoked by `git worktree add` after the worktree has been
created its files have been checked out. It receives a single argument
which is the path to the newly created worktree. The exit code of the
hook is ignored.
