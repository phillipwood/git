...
static GIT_PATH_FUNC(rebase_path_state, "rebase-merge/state")
				  ...
				  unsigned flag)
{
	...
	ctx->pending_squash = command == TODO_SQUASH;
...
static int read_populate_opts(struct repository *r, struct replay_opts *opts)
{
	...
	if (is_rebase_i(opts)) {
		...
		if (ctx->current_fixup_count) {
			if (strbuf_read_file(&ctx->fixup_msg,
		    ...
		    const struct todo_list *todo_list)
{
	...
	if (commit) {
		...
		ret = write_message(ctx->message.buf, ctx->message.len,
				    git_path_merge_msg(r), 0);
		strbuf_release(&buf);
		if (ret) {
			error_errno(_("could not write '%s'"),
