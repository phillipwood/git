...
test_expect_success C_LOCALE_OUTPUT 'getting non-existent alias fails' '
	test_must_fail git alias does-not-exist 2>actual &&
	echo "error: alias ${SQ}does-not-exist${SQ} does not exist" >expect &&
	test_cmp expect actual
'

test_expect_success C_LOCALE_OUTPUT 'getting empty alias fails' '
...
test_expect_success C_LOCALE_OUTPUT 'setting an alias to the name of a builtin fails' '
	test_must_fail git alias checkout some other command 2>actual &&
	echo "error: ${SQ}checkout${SQ} is a git command" >expect &&
	test_cmp expect actual
'
test_expect_success C_LOCALE_OUTPUT 'setting an alias to the name of an external script' '
...
test_expect_success C_LOCALE_OUTPUT 'setting an empty alias fails' '
	test_must_fail git alias mycmd "   " 2>actual &&
	echo "error: alias command is empty" >expect &&
	test_cmp expect actual
'
